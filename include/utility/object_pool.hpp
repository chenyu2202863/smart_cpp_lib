...
namespace utility { 
	...
	struct object_pool_t
	{
		...
		typedef std::function<void(value_t &)> clear_handler_t;
		...
		clear_handler_t clear_handler_;
		pool_t pool_;

		object_pool_t(const create_handler_t &create_handler, const clear_handler_t &clear_handler = nullptr)
			: pool_(std::make_shared<queue_t>())
			, create_handler_(create_handler)
			, clear_handler_(clear_handler)
		{
