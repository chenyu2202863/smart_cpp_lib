...
public:
    ...
    bool run_test_(const tests_iterator& ti, safe_holder<object>& obj, test_result &tr)
    {
        ...
        catch (const rethrown_t<char>& ex)
        {
            tr = ex.tr;
            tr.result = test_result::rethrown;
        }
		catch (const tut_error_t<char>& ex)
		{
			tr.result = ex.result();
			tr.exception_typeid = ex.type();
			tr.message = ex.what_();
		}
		catch (const tut_error_t<wchar_t>& ex)
		{
			tr.result = ex.result();
			tr.exception_typeid = (const char *)CW2A(ex.type().c_str());
			tr.message = (const char *)CW2A(ex.what_().c_str());
		}
        catch (const std::exception& ex)
        {
