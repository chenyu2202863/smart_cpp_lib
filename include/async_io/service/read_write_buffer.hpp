...
namespace async { namespace service {
	...
	public:
		mutable_buffer_t()
			...
			, size_(0)
		{}
		mutable_buffer_t(value_type *data, std::uint32_t size)
			: data_(data)
			, size_(size)
		{}
	...
	inline mutable_buffer_t operator+(const mutable_buffer_t &buffer, std::uint32_t sz)
	{
		...
		std::uint32_t newSize = buffer.size_ - sz;

		return mutable_buffer_t(newData, newSize);
	}
	inline mutable_buffer_t operator+(std::uint32_t sz, const mutable_buffer_t &buffer)
	{
		return buffer + sz;
	}
	...
	public:
		const_buffer_t()
			...
			, size_(0)
		{}
		const_buffer_t(const value_type *data, std::uint32_t size)
			: data_(data)
			, size_(size)
		{}
	...
	inline const_buffer_t operator+(const const_buffer_t &buffer, std::uint32_t sz)
	{
		assert(sz <= buffer.size_);
		if( sz > buffer.size_ )
			throw service::network_exception("sz > buffer.size_");
	...
	inline const_buffer_t operator+(std::uint32_t sz, const const_buffer_t &buffer)
	{
		return buffer + sz;
	}
	...
	inline const_buffer_t buffer(const std::pair<char *, std::uint32_t> &data)
	{
		return const_buffer_t(data.first, data.second);
	}

	inline const_buffer_t buffer(const std::pair<const char *, std::uint32_t> &data)
	{
		return const_buffer_t(data.first, data.second);
	}
	...
	{
		return const_buffer_t(data.data(), (std::uint32_t)data.size());
	}	
