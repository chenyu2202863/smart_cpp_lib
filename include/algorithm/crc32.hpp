#ifndef __ALGORITHM_CRC32_HPP
#define __ALGORITHM_CRC32_HPP
...
namespace algorithm
{
	namespace crc
	{
		...
		public:
			inline crc32& add(void const *pbuf, std::uint64_t len) 
			{
				unsigned char *buffer = (unsigned char *)pbuf;
				while(len--)
					crc_ = (crc_ >> 8) ^ detail::Crc32Table[(crc_ & 0xFF) ^ *buffer++];
				return *this;
			}
		...
		inline std::uint32_t cac_crc32(const void *pbuf, std::uint64_t size)
		{
			return crc32().add(pbuf, size).get();
		}
	}
