...
namespace win32 { namespace service {

	service_t* service_t::m_pThis = NULL;

	service_t::service_t(LPCTSTR lpszSvrName, LPCTSTR lpszSvrDispName, LPCTSTR lpszsvrPath,
	...
	DWORD service_t::InstallService(LPCTSTR lpszSvrName, LPCTSTR lpszSvrDispName)
	{
	...
	BOOL service_t::CheckService(LPCTSTR lpszSvrName, LPCTSTR lpszSvrDispName, LPCTSTR lpszSvrExePath)
	{
	...
	DWORD service_t::RemoveService(LPCTSTR lpszSvrName)
	{
	...
	bool service_t::HasService(LPCTSTR lpszSvrName)
	{
		SC_HANDLE hSCM = NULL;
	...
	bool service_t::IsServiceRunning(LPCTSTR lpszSvrName)
	{
	...
	DWORD service_t::StartService(LPCTSTR lpszSvrName)
	{
		SC_HANDLE hSCM = NULL;
		SC_HANDLE hService = NULL;
	...
	DWORD service_t::StopService(LPCTSTR lpszSvrName)
	{
		DWORD dwError = 0;
		SC_HANDLE hSCM = NULL;
		SC_HANDLE hService = NULL;
	...
	DWORD service_t::GetWin32ErrorText(LPTSTR buf, DWORD dwSize, DWORD dwError /* = GetLastError */)
	{
	...
	void service_t::ShowHelp()
	{

	}

	DWORD service_t::ChangeConfig()
	{
	...
	DWORD service_t::ProcessCmdLine(int argc, TCHAR *argv[])
	{
		service_t* pSvr = AfxGetWin32Svr();
		DWORD dwError = ERROR_SUCCESS;
	...
	DWORD service_t::ReportStatusToSCM(DWORD dwStatus)
	{
		if( m_bDebug )
		{
	...
	DWORD  service_t::AddLogToEvent(LPCTSTR fmt, ...)
	{
	...
	DWORD  service_t::AddLogToEvent(DWORD dwError /* = GetLastError */)
	{
	...
	void __stdcall service_t::ServiceCtrlHandler(DWORD dwControl)
	{
		service_t* pSvr = AfxGetWin32Svr();
		switch( dwControl )
		{
	...
	void __stdcall service_t::ServiceMain(DWORD dwArgc, LPTSTR* lpszArgv)
	{
		service_t* pSvr = AfxGetWin32Svr();
	...
	BOOL __stdcall service_t::ConsoleHandler(DWORD dwCtrlType)
	{
		switch( dwCtrlType )
		{
