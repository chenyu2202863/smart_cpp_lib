...
{	
	...
	http_error_t::http_error_t(DWORD error)
		: runtime_error("")
		, error_(error)
	{
		http_error_info::error_info_table_type::const_iterator iter = error_info.errors_.find(error);

		if( iter != error_info.errors_.end() )
		{
			std::ostringstream os;
			os << "Win32 Error(" << error_ << ") " << iter->second;
			msg_ = os.str();
		}
		else
		{
	...
	http_error_t::http_error_t(const char *msg)
		: runtime_error(msg)
		, msg_(msg)
	{
	...
	http_error_t::http_error_t(const error_code &code)
		: runtime_error(0)
	{
		http_error_code::error_code_table_type::const_iterator iter = error_code_.errors_.find(code);
